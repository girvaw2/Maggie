...
{
  pub_ = nh_.advertise<std_msgs::String>("topic_name", 5);
  testTimer_ = nh_.createTimer(ros::Duration(1), &HeadGestures::testTimerCallback, this);
  testTimer_.stop(); // off by default.
...
HeadGestures::processNod()
{
  if (vTrajectory_.size() > 0)
  {
	...
	std::cout << "Tilt Up: currTiltAngle_ = " << currTiltAngle_ << " tp.angle = " << tp.angle << " x = " << 10 * cos(tp.angle) << " z = " << 10 * sin(tp.angle) << std::endl;
      ...
      {
	setHeadPosition("base_link", 10 * cos(tp.angle), 0, 10 * sin(tp.angle));
      }
      else
      {
	vTrajectory_.erase(vTrajectory_.begin());
...
void 
HeadGestures::testTimerCallback(const ros::TimerEvent& event)
{
  //DoTest();
} 
